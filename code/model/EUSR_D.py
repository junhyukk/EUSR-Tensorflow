from model.base_model_ import BaseModel
import tensorflow as tf

def create_model(args, parent=False):
    return EUSR_D(args)

class EUSR_D(BaseModel):
	def __init__(self, args):
		super(EUSR_D, self).__init__(args)
		self.build_model()
		self.init_saver()
		num_params = self.count_num_trainable_params()
		print("num_params: %d" % num_params)
		print("EUSR_D and Saver are initialized!")
	...
	def build_model(self):
		...
		x = res_module(in_img, 3, 3)
		x = tf.concat([in_img, x], axis=3)

        # First convolution layer
		x = conv(x, num_feats, [3,3])

        # Scale-specific processing module
		x = tf.cond(tf.equal(self.flag_scale, 1), lambda: scale_specific_module(x, num_feats, [5,5]), \
        lambda: tf.cond(tf.equal(self.flag_scale, 2), lambda: scale_specific_module(x, num_feats, [5,5]), \
		...
		x = tf.cond(tf.equal(self.flag_scale, 1), lambda: scale_specific_upsampler(x, 4), \
		lambda: tf.cond(tf.equal(self.flag_scale, 2), lambda: scale_specific_upsampler(x, 4), \
		lambda: scale_specific_upsampler(x, 4)))
